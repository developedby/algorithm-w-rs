...
newtype Program = Program (Map.Map String (Bool, Expr))
...
inferProgram :: TypeEnv -> Map.Map String (Bool, Expr) -> [String] -> Infer [(String, Type)]
...
elaborate program@(Program fns) = do
  -- let order = Map.keys fns
  let order = topologicalOrder program
  types <- evalState (runExceptT $ inferProgram Map.empty fns order) 0
...
buildDependencyGraph (Program fns) =
  let nodes = Map.toList fns
      ...
      getEdges (_, (_, expr)) = collectReferences expr
      graph =
        array
          (0, length nodes - 1)
...
instance Show Program where
  show (Program fns) = unlines [name ++ " = " ++ show expr | (name, (rec, expr)) <- Map.toList fns]
